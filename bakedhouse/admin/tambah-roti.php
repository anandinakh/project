<?php
    session_start();
    require "../koneksi.php";
    $queryKategori = mysqli_query($koneksi, "SELECT * FROM roti");
    ...
    if(isset($_POST['tambah'])){
        ...
        $path = "../assets/img/roti/".$nama_file;
        $pathdb = "".$nama_file;
        ...
        if($tipe_file == "image/jpeg" || $tipe_file == "image/png"){
          if($ukuran_file <= 5000000){ 
            if(move_uploaded_file($tmp_file, $path)){ 
              $sql = $koneksi ->query("insert into roti (gambar, nama, harga, deskripsi)
              values('$pathdb','$nama','$harga', '$deskripsi')");

            header("location:admin-roti.php");
            }else{
